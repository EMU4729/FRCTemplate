...
public class CrvFt {
  ...
  /** fits curve and adjusts range of input */
  public CrvFt(double inMin,double inMax,double outAbsMin,double outAbsMax,double pow){
    if(pow <=0){throw new IllegalArgumentException("pow must be a positive non zero double");}
    this.inMin = inMin;
    this.inMax = inMax;
   ...
   * (pow > 1 = slow start quick end, pow = 1 = linear, pow < 1 = quick start slow end)
   * adjusts range again so that the output jumps straight to outAbsMin and curves up to outAbsMax 
  ...
  public double fit(double input, double maxOutAdjust){
    ...
    input = Math.copySign(Math.abs(input)*(-outAbsMin+outAbsMax*maxOutAdjust) + outAbsMin,input);      //abjust min starting point and end point either side of zero
    return input;
  }
}
